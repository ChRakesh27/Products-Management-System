...
function ProductionData({ poData }) {
      ...
      <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-2  border-black/50 p-2 bg-white">
              {poData?.products.map((opt) => {
                const id = `plan-${opt.id}`;
                const checked = selectedProduct === opt.id;

                return (
                  <div key={opt.id} className="flex-1">
                    {/* native radio (hidden visually, accessible) */}
                    <input
                      id={id}
                      type="radio"
                      name="plan"
                    ...
                    <label
                      htmlFor={id}
                      className={[
                        "block w-full text-center rounded-md px-4 py-2 text-xs font-semibold transition-transform",
                        "cursor-pointer active:scale-95",
                        checked
                          ? "bg-black text-white shadow-[0_0_10px_0_rgba(0,0,0,0.5)]"
