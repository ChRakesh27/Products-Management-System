...
const emptyVariant = (): RawMaterialVariantModel => ({
  id: generateUUID(),
  size: "",
  color: "",
  quantityOrdered: 0,
...
export default function SetRawMaterial() {
  ...
  const [variants, setVariants] = useState<RawMaterialVariantModel[]>([
    emptyVariant(),
  ]);
  ...
  useEffect(() => {
    const run = async () => {
      ...
      setLoading(true);
      try {
        const item = await materialsAPI.get(id);
        if (item) {
          setName(item.name || "");
          ...
          setVariants(
            item.variants?.length
              ? item.variants.map((v) => ({
                  ...
                  quantityUsed: Number(v.quantityUsed) || 0,
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEdit && id) {
        ...
        const saved = await materialsAPI.create(payload);
        nav(`/materials/${saved.id}`);
      }
    } finally {
      setSaving(false);
    }
    ...
    <div className="mx-auto max-w-7xl p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm text-muted-foreground">
          {isEdit ? "Edit" : "Create"} Raw material
        </div>
      </div>
                    ...
                    >
                      ...
                      <div className="">
                        ...
                        <Input
                          ...
                          value={v.unitPrice}
                          onChange={(e) =>
                            updateVariant(v.id, "unitPrice", e.target.value)
                          }
                          min={0}
                        />
                      </div>
                      <div className="">
                        <Label>Total</Label>
                        <div className="h-10 rounded-md border bg-muted px-3 flex items-center  font-medium">
                          {currency(v.total)}
                        </div>
                      </div>
